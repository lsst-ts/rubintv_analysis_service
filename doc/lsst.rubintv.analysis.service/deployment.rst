...
rather than the mounted secret path. The schema files are installed from PyPI
as in production; to use a local ``sdm_schemas`` checkout instead, pass
``--schemas-dir <checkout>/yml``.
