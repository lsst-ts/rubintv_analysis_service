...
A result is passed through ``to_json_safe`` in ``command.py`` before it is encoded, so a command can return what the database driver, numpy or the stack hands it and need not convert values itself. This is the contract a client can rely on for each ConsDB ``datatype``; the React client reads it in ``src/model/columnData.ts`` and ``src/model/schema.ts`` of ``rubintv-ddv``.

- ``int``, ``long``: sent as JSON integers, never widened to a float. The integer/float distinction matters to a client for formatting axes and is otherwise not recoverable from the values, so it is carried by the schema ``datatype`` and the worker keeps to it.
- ``float``, ``double``: sent as JSON numbers, and always finite. A float that is ``NaN`` or infinite is a problem: Python would write the bare tokens ``NaN`` and ``Infinity``, which are not JSON and make a strict decoder (Dart's ``jsonDecode``, and ``JSON.parse`` in a browser) reject the whole message. A "load columns" query therefore drops any row holding one in a requested column, exactly as it already excludes rows where a column is ``NULL``, and any such value left elsewhere (an aggregate of an all-``NaN`` column, say) is sent as ``null``. The count that precedes a load can include those rows and so slightly overstate what arrives.
- ``string``, ``text``, ``boolean``: sent as JSON strings and booleans.
- ``timestamp``: the driver returns a ``datetime``, which is sent as ``YYYY-MM-DD HH:MM:SS`` with ``.ffffff`` appended when there is a fractional second, e.g. ``2023-05-19 20:20:20.123456``. The separator is a space and there is no zone suffix: ConsDB stores TAI without a zone, and this is the form both clients parse (the React client swaps the space for ``T``, appends ``Z`` and uses ``Date.parse``, which keeps the first three fractional digits; the Flutter client's ``dateFromString`` splits on the space). An aware datetime from elsewhere is converted to UTC and its zone dropped. A ``date`` or ``time`` is sent as its ISO string and a ``timedelta`` as seconds.
- numpy scalars and arrays become the equivalent Python values, ``Decimal`` a float, ``UUID`` a string, tuples and sets lists.

Anything else fails in ``json.dumps`` and is reported to the client as a ``command response error`` rather than being stringified, so a new value type shows up as an error instead of a wrongly-typed column. ``tests/test_flutter_contract.py`` pins the timestamp format and the absence of ``NaN``; change a client's parsing and that test together.
